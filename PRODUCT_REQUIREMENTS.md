...
| NFR-84 | MUST | A page title MUST be escaped exactly once, at the point it is rendered. A controller MUST NOT hand `render()` a pre-escaped title that the layout then escapes again: a course called `Ölçme & Değerlendirme` currently reaches the browser as `Ölçme &amp;amp; Değerlendirme`. The fix changes rendered bytes wherever a title contains `&`, `'` or `"`, so it MUST be applied to every HTML route in one change rather than route by route. |
