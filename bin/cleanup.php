...
if (!empty($toDelete)) {
    ...
    if ($dryRun) {
        foreach ($toDelete as $s) {
            echo sprintf('  [dry-run] Would delete session id=%d (%s) requested_at=%s%s',
                $s['id'], $s['short_code'], $s['delete_requested_at'], PHP_EOL);
        }
    } else {
        ...
        foreach ($toDelete as $s) {
            ...
            $deleted++;
            echo sprintf('  [cleanup] Deleted session id=%d (%s)%s',
                $s['id'], $s['short_code'], PHP_EOL);
        }
...
if (!empty($toAnon)) {
    ...
    if ($dryRun) {
        foreach ($toAnon as $s) {
            echo sprintf('  [dry-run] Would anonymize session id=%d (%s) closed_at=%s%s',
                $s['id'], $s['short_code'], $s['closed_at'], PHP_EOL);
        }
    } else {
        ...
        foreach ($toAnon as $s) {
            ...
            $anoned++;
            echo sprintf('  [cleanup] Anonymized session id=%d (%s)%s',
                $s['id'], $s['short_code'], PHP_EOL);
        }
