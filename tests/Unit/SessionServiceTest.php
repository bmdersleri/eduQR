...
class SessionServiceTest extends TestCase
{
    ...
    public function testShortCodeUsesOnlyAllowedCharset(): void
    {
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            for ($j = 0; $j < strlen($code); $j++) {
                $this->assertStringContainsString($code[$j], $charset,
    ...
    private function makeCourseRepo(array $courses = []): CourseRepositoryInterface
    {
        return new class($courses) implements CourseRepositoryInterface {
            ...
            public function create(int $instructorId, string $title, ?string $code, ?string $semester, ?string $description, string $defaultLanguage): int { return 1; }
